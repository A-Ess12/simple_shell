...
/**
 * isCommandChainDelimiter - tests if the current character in
...
/**
 * checkChainStatus - checks if we should continue chaining
...
void checkChainStatus(info_t *info, char *buf, size_t *p, size_t i, size_t len)
{
	...
	if (info->commandChainType == CMD_AND && info->status)
	{
		buf[i] = 0;
		j = len;
	}
	if (info->commandChainType == CMD_OR && !info->status)
	{
		buf[i] = 0;
		j = len;
	}
...
int replaceAliases(info_t *info)
{
	...
	for (i = 0; i < 10; i++)
	{
		node = findAlias(info->alias, info->argv[0]);
		if (!node)
			return (0);
		free(info->argv[0]);
		p = _strchr(node->str, '=');
...
int replaceVariables(info_t *info)
{
	...
	for (i = 0; info->argv[i]; i++)
	{
		...
		node = findVariable(info->env, &info->argv[i][1]);
		if (node)
		{
			replaceString(&(info->argv[i]),
