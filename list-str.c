 ...
 * @strValue: str field of the new node
 * @numValue: node index used by history
 *
 * Return: a pointer to the new head of the list
 */
list_t *addNodeToStart(list_t **head, const char *strValue, int numValue)
{
	list_t *newHead;
	...
	newHead->num = numValue;

	if (strValue)
	{
		newHead->str = _strdup(strValue);
		if (!newHead->str)
		{
			free(newHead);
			return (NULL);
 ...
 */
list_t *addNodeToEnd(list_t **head, const char *strValue, int numValue)
{
	list_t *newNode, *node;
	...
	newNode->num = numValue;

	if (strValue)
	{
		newNode->str = _strdup(strValue);
		if (!newNode->str)
		{
			free(newNode);
			return (NULL);
